...
import sys
import traceback
print(f"DEBUG: app.py starting. CWD: {os.getcwd()}")
print(f"DEBUG: Directory Contents: {os.listdir()}")

try:
    import pulsevox # <-- SIMPLE IMPORT
    st.success("DEBUG: Successfully imported 'pulsevox' module!") # Success message
    print("DEBUG: Successfully imported 'pulsevox' module!") # Also print to logs
    # Access a simple variable to be sure
    print("DEBUG: Accessing pulsevox.TASK_FILE:", pulsevox.TASK_FILE)

except ImportError as e_import:
    st.error(f"FATAL: Failed to import 'pulsevox' module itself. Error: {e_import}")
    st.error(traceback.format_exc()) # Show full traceback if import fails
    st.stop()
except Exception as e_generic:
    st.error(f"FATAL: An unexpected error occurred during 'import pulsevox': {e_generic}")
